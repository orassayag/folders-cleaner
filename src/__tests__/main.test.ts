...
import { main, settings, Scanner, Cleaner } from '../index.js';
import * as pathValidator from '../utils/index.js';
...
describe('main', () => {
  ...
  it('should run successfully when folders are found and cleaned', async () => {
    vi.mocked(pathValidator.validateAndResolvePath).mockResolvedValue(
      '/test/path'
    );
  ...
  it('should exit with 0 when no folders are found', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining('No subdirectories found')
    );
    expect(processExitSpy).toHaveBeenCalledWith(0);
  });
  ...
  it('should show error and exit with 1 when settings are invalid (empty)', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining('Configuration Required')
    );
